...
CREATE TABLE CIA_PERMISSAO_FUNCIONALIDADE (
    TIPO_PERFIL VARCHAR(2) NOT NULL,

    CHAVE_FUNCIONALIDADE VARCHAR(200) NOT NULL,

    INDI_HABILITADO VARCHAR(1) DEFAULT 'N' NOT NULL,
...
CREATE OR REPLACE FUNCTION trg_bu_cia_perm_func_fn()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_bu_cia_perm_func
  BEFORE UPDATE ON cia_permissao_funcionalidade
  FOR EACH ROW EXECUTE FUNCTION trg_bu_cia_perm_func_fn();
